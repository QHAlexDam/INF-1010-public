...
int main()
{
   ...
   //  assigner un chiffre d'affaire a Poly2
    ...
    if (poly.getChiffreAffaire() < poly2.getChiffreAffaire())
        cout << "polyFood2 gagne plus d'argent" << endl;
    else
         cout << "polyFood gagne plus d'argent" << endl;
	...
	//Ces delete n'etaient pas dans le main original, mais sont necessaire pour regler les fuites de memoire. notamment, si menu1 
	//n'est pas efface, cela cause des fuites de memoire dans les plats, car ceux-ci sont efface par le destructeur de menu.
	delete menu1;
