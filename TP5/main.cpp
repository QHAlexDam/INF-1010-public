...
// Q1)Aurait-il été possible de créer un autre gestionnaire pour les autres classes? Si oui,
//	laquelle et quel type de conteneur conseilleriez - vous ?
//	
//		On aurait pu faire un gestionnaire pour les clients aussi. Cela permettrait une gestion 
//		plus facile des differents types de clients. Un conteneur qu'on pourrait utiliser est un
//		map comme pour les plats dans lequel il y aurait des pair<string nom, Client*>. Ce conteneur
//		permettrait d'identifier les clients par leur nom. Il pourrait avoir un probleme dans le cas
//		de clients possedant le meme nom. L'ideal serait alors de mapper les clients avec un id unique.
//
// Q2)Pourquoi l’implémentation des classes génériques est dans .h et non pas séparée en .h
//	et.cpp comme les classes normales ?
//
//		Les classes generiques sont implementes directement dans le .h pour des raisons de compilation
//		si l'implementation est separee comme une classe normale, le compilateur ne peut pas savoir le
//		type du template et il y aura une erreur. Par exemple, la classe GestionnaireTable qui inclue 
//		le gestionnaireGenerique doit pouvoir acceder a l'implementation et si l'implementation est 
//		separee dans un .cpp, le compilateur ne le trouve pas necessairement.
